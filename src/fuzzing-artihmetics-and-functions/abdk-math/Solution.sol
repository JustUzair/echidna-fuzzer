...
contract Solution {
    ...
    function toInt(int128 x) private pure returns (int64) {
    ...
    // (x / y) != (y / x)
    function div_test_not_commutative(int128 x, int128 y) public pure {
        // @note - if the function is to be tested, excluding the same value, uncomment next line
        // require(abs(x) != abs(y));
        // emit Debug(x, y);
        int128 x_div_y = div(x, y);
        int128 y_div_x = div(y, x);
        // emit Debug(x_div_y, y_div_x);

        if (abs(x) == abs(y)) {
            assert(x_div_y == y_div_x);
        } else {
            assert(x_div_y != y_div_x);
        }
    }
